...
3. Install Nix either with Lix or the Determinate Systems installer; if you're using the Determinate installer then explicitly say NO to Determinate Nix, as it isn't compatible with `nix-darwin`.[^determinate-vs-lix]
...
[^determinate-vs-lix]: Although I'd prefer to use Lix in all cases, it isn't compatible with `nixd` (the Nix language server) and might cause some other issues related to experimental Nix features.
